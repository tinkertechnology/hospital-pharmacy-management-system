...
class GeneratePDF(APIView):
	def get(self, request, cart_id, *args, **kwargs):
		...
		total_in_words = generate_amount_words(0)	
		total_in_number  = cart.total - cart.transaction_total
		if total_in_number > 0:
...
class GenerateFullPDF(APIView):
	def get(self, request, cart_id, *args, **kwargs):
		...
		template = get_template('orders/invoice_full.html')
		context = {
			'carts': carts,
			# 'items': items,
			'total_in_words':total_in_words,
